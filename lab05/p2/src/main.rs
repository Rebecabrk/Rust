...
fn new_canvas() -> Canvas{
    let c = Canvas{matrix : [[' '; 50]; 10]};
    c
}

fn set_pixels(c: &mut Canvas , a:&[(u8, u8, u8)]) -> () {
    for i in 0..a.len(){
        let x = a[i].0 as usize;
        let y = a[i].1 as usize;
        let ch : char = a[i].2 as char;
        c.matrix[x][y] = ch;
    }
}

fn print(c: Canvas) -> (){
    for i in 0..c.matrix.len(){
        for j in 0..c.matrix[i].len(){
            print!("{}",c.matrix[i][j]);
        }
        println!("");
    }
